...
- Label progress on each question (e.g., `Context Q1/5`).
- With each question, offer **numbered response options and a recommended default**; include `Other (specify)` when useful.
- Handle interruptions gracefully and allow the user to pause and resume.
...
3. **Which existing skills does this reference?** (Search your existing skills for any that already cover this topic before proceeding.)
...
2. **Validate the frontmatter** — Confirm `name` and `description` are present and within limits (name ≤ 64 chars kebab-case, description ≤ 200 chars with a "Use when..." trigger cue).
3. **Confirm structure and triggers** — Verify all required sections are present and the description/triggers would actually match real user requests.
4. **Check for overlap** — Make sure this doesn't duplicate an existing skill.
